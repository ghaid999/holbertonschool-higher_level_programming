...
A Flask API demonstrating two authentication schemes:
- HTTP Basic Authentication (via Flask-HTTPAuth)
- JWT-based authentication (via Flask-JWT-Extended), including a
  simple role-based access control check for admin-only routes.
"""
...
def verify_password(username, password):
    """Verify basic-auth credentials against the in-memory users store."""
...
@jwt_required()
def admin_only():
    """Route accessible only to users whose role is 'admin'."""
...
def handle_invalid_token_error(err):
    """Handle requests with a malformed or invalid JWT token."""
